...
export namespace Helpers {
	...
	export function _update(dbConnection, collectionName, id, payload) {
		...
		return new Promise((resolve, reject) => {
			dbConnection.db(databaseName)
				.collection(collectionName, this.fetchCollection)
				.updateOne({"_id": id}, {$set: payload}, {upsert: false}).then(
				result => {
					resolve(result);
					dbConnection.close();
				}, err => {
	...
	export function _formatQuery(queryParams: QueryParameters) {
		...
		for (let property in queryParams) {
			if (queryParams.hasOwnProperty(property) && invalidFields.indexOf(property) === -1) {
	...
	export function formatQueryParams(queryParams: QueryParameters) {
		...
		if (queryParams.fields || queryParams.omitFields) {
			...
			projection = this._formatParams(queryParams.fields || queryParams.omitFields, show);
		}
		...
		return {query: query, projection: projection, skip: skip, limit: limit, sort: sort, embed: embed};
	}
}
